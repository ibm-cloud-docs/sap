...
To provide the access for the required roles to run the "SAP HANA db backup to Cloud Object Storage", go to https://cloud.ibm.com/iam/users and select the USER_ID to run the automation.
   ...
   ![Figure 1. SAP HANA db backup to Cloud Object Storage](../../images/vpc-intel-vsi-grant-permissions-cos.png "SAP HANA db backup to Cloud Object Storage"){: caption="SAP HANA db backup to Cloud Object Storage" caption-center="bottom"}
...
*	[Create or retrieve an {{site.data.keyword.cloud_notm}} API key](/docs/iam?topic=iam-userapikey&interface=ui#create_user_key){: external}. The API key is used to authenticate with the create or retrieve your SSH key ID.
...
5. Run a full DATA backup test for SYSTEM and Tenant DBs using the new backup location (file/disk, other backup tool), different from {{site.data.keyword.cloud_notm}} Object Storage. Make sure that all backups complete successfully in the new location.
