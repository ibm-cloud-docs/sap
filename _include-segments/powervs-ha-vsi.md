...
Prepare your {{site.data.keyword.cloud_notm}} and {{site.data.keyword.powerSys_notm}} environment for a high availability cluster by configuring the workspaces, IAM access, and collecting required cluster parameters.
{: shortdesc}
...
- For Red Hat Enterprise Linux (RHEL), see [Implementing high availability for SAP applications on IBM {{site.data.keyword.powerSys_notm}} References](/docs/sap?topic=sap-ha-rhel-refs).
- For SUSE Linux Enterprise Server (SLES), see [Implementing high availability for SAP applications on IBM {{site.data.keyword.powerSys_notm}} SLES References](/docs/sap?topic=sap-ha-sles-refs).
...
1. Create a workspace.
    ...
    After you create the workspace, it appears under **Workspaces** in the left navigation panel of the {{site.data.keyword.powerSys_notm}} user interface.
1. Create network subnets in the workspace.

    Later, you attach the virtual server instances to the subnet, and they receive an IP address from the configured subnet range.
...
1. Select a boot image.
    ...
    If you want to import a custom Linux image, first upload the image to {{site.data.keyword.cos_full}} in OVA format. For more information, see the following topics:
    ...
    Provide the OVA image in the {{site.data.keyword.cos_full}} bucket before you start provisioning virtual server instances.
...
When the cluster runs on {{site.data.keyword.powerSys_notm}} instances, the agents use the IBM Power Cloud API to retrieve status information and trigger required actions.
Configure {{site.data.keyword.iamshort}} (IAM) to authenticate and authorize these API calls.

Create a custom role that restricts agent actions to the minimum required set.

A service ID in {{site.data.keyword.cloud_notm}} is a unique identifier that enables applications and services to authenticate with IBM Cloud services.
Create a service ID for the agents so that they can access IBM Power Cloud actions such as monitoring or controlling virtual server instances.
...
1. Select **Manage** > **Access (IAM)** > **Users** from the menu bar, and then click your username.
...
1. Enter the **Role Name**, **Role ID**, and **Role description** for the custom role.
1. Select **Workspace for {{site.data.keyword.powerSys_notm}}** from the **Service** list.
1. Set **View** to **Manager** and **Type** to **Service**, and then search for `power-iaas.pvm-instance.` to simplify the list.
1. In the **Actions** list, select the following actions.
...
1. Enter the **Role Name**, **Role ID**, and **Role description** for the custom role.
1. Select **Workspace for {{site.data.keyword.powerSys_notm}}** from the **Service** list.
1. Set **View** to **Manager** and **Type** to **Service** to simplify the list.
1. Select the following actions in the **Actions** list.
...
1. Enter the **Role Name**, **Role ID**, and **Role description** for the custom role.
1. Select **Workspace for {{site.data.keyword.powerSys_notm}}** from the **Service** list.
1. Set **View** to **Manager** and **Type** to **Service**, and then search for `route` to simplify the list.
1. Select the following actions in the **Actions** list.
   - power-iaas.route.read
   - power-iaas.route.update
1. Click **Create** to save the role.
...
Collect the following parameters before you proceed.
...
If you use the `fence_ibm_powervs` fence agent or the `powervs-subnet` resource agent, also collect the following information:
   - `IBMCLOUD_CRN_1`, `IBMCLOUD_CRN_2`. The *Cloud Resource Name (CRN)* of the {{site.data.keyword.powerSys_notm}} workspace or workspaces.
   - `POWERVSI_1` and `POWERVSI_2`. The virtual server *instance IDs*.
   - `GUID_1`, `GUID_2`. The *GUID* values that are derived from the corresponding CRNs (required by `fence_ibm_powervs` only).
...
- `CLOUD_REGION` specifies the deployment region of the virtual server instances and is used to identify the appropriate [Power Cloud API endpoint](https://cloud.ibm.com/apidocs/power-cloud#endpoint){: external}.
   ...
   See [IBM Cloud&reg; regions](/docs/power-iaas?topic=power-iaas-ibm-cloud-reg) for the zone mapping.

- `IBMCLOUD_CRN` and `GUID`.
   ...
   In a multizone region deployment, use the *CRN* of the second workspace and note the contents for `IBMCLOUD_CRN_2` and `GUID_2`.

- `POWERVSI`, the IDs of the virtual server instances
...
- `ROUTE_CRN`, the CRNs of the static routes (`powervs-move-ip` resource agent only).
   ...
   Select the static route that matches the destination of the virtual IP address.
...
- `APIKEY`, the API key for the fencing agent.
   ...
   For deployments in a multizone region, an API key is also required for the `powervs-move-ip` or `powervs-subnet` cluster resource agents.
   As with the fencing agent, you can use the value of the `apikey` entry for the `APIKEY` parameter.
   However, the recommended approach is to store a copy of the downloaded JSON key file on both nodes and configure `APIKEY` to reference the file by using an `@` prefix followed by the absolute path to the key file.


## Creating virtual server instances
{: #ha-vsi-create-virtual-server-instances}

After you prepare the workspace, subnets, SSH key, and boot image, create the virtual server instances that will serve as nodes in the high-availability cluster.
For provisioning steps, refer to either [Deploying an {{site.data.keyword.powerSys_notm}} instance for the SAP HANA system](/docs/sap?topic=sap-powervs-set-up-power-instances#powervs-set-up-power-hana-instance) or [Deploying an {{site.data.keyword.powerSys_notm}} instance for SAP NetWeaver](/docs/sap?topic=sap-powervs-set-up-power-instances#powervs-set-up-power-netweaver-instance).
