...
# Access VPC File Storage from IBM {{site.data.keyword.powerSys_notm}} instances that use a Network Load Balancer with routing mode
{: #ha-nlb-rt-nfs-intro}
...
See the list of [required permissions](/docs/vpc?topic=vpc-managing-user-permissions-for-vpc-resources) for VPC.

- You need an SSH key to connect to the virtual servers instances.
...
1. Enter `nfs-server-sg` for the name. Select the same **Resource group** as the VPC resource group.
...
To support routing mode, you must first create a service-to-service authentication policy for your NLB.
   ...
   1. For the source account, select **This account**, and click **Next**.
   1. For the source service, select **VPC Infrastructure Services**.
      For scope access, select **Specific resources** > **Resource Type** > **Load Balancer for VPC**.
   1. For the target service, select **VPC Infrastructure Services**.
      For scope access, select **Specific resources** > **Resource Type** > **Virtual Private Cloud**.
...
3. Under the **Location** section, select the same **Geography** and **Region** that are used for the virtual private cloud.
...
5. Click **Attach server** to add server instances to the pool.
   ...
   - Select the instances that you created, and set `22` as the **port**.
...
1. Note down the first entry of the list of **Private IPs**.
...
1. Note down the **Primary IP** and the **Mount path** details.

Later on, the parameter **Destination** in the VPC route entry is configured as the **Primary IP** of the file storage share.
The parameter **Mount path** is specified as argument for the `mount` command on the IBM {{site.data.keyword.powerSys_notm}} instance.
...
1. Click **Create**.
1. Under the **Location** section, select the same **Geography**, **Region** that are used for the virtual private cloud.
